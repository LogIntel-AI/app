<x-app-layout>
    ...
    <div class="py-12 relative z-10 animate-fade-in">
                        ...
                        @forelse($servers as $server)
                            <div class="bg-gray-900/50 border border-gray-700 p-4 rounded-lg flex flex-col justify-between items-start gap-4 hover:border-gray-600 transition">
                                ...
                                <div class="w-full bg-black/40 p-3 rounded border border-gray-800 flex items-center justify-between">
                                    ...
                                    <button onclick="navigator.clipboard.writeText('{{ $server->api_token }}'); alert('API Key copied!')" class="ml-4 text-gray-400 hover:text-white transition" title="Copy to clipboard">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                                    </button>
                                </div>
                                <p class="text-xs text-gray-500 w-full">Send POST requests to <code class="text-purple-400">{{ url('/api/logs/ingest') }}</code> with this Bearer token.</p>
                            </div>
                        @empty
